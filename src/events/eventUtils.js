...
/* [FORMAT] [EVENT] Returns "Roll Die" for 1 die, "Roll Dice" for multiple. */
export function getEventRollButtonLabel(diceCount) {
  return diceCount === 1 ? "Roll Die" : "Roll Dice";
}

/* [EVENT] [FORMAT] Appends text to an existing event summary string with a space separator. */
...
/* [FORMAT] [EVENT] Converts a token-placement location enum to a readable English phrase. */
...
/* [FORMAT] [EVENT] Converts a move-destination enum to a readable English phrase. */
...
/* [FORMAT] [EVENT] Converts a single event effect object to a human-readable sentence. */
...
/* [FORMAT] [EVENT] Converts an event effect object to a human-readable single sentence. */
...
/* [PATHFINDING] [MOVEMENT] Returns all tiles door-adjacent or stair-connected to the player's current tile. Used by mask-push and other range checks. */
...
/* [EVENT] [TILE-PLACEMENT] [LOOKUP] Returns all valid placement options for a move/place-token event effect destination. Handles named landmarks, floor filters, adjacency, and token-type exclusions. */
...
/* [FORMAT] [EVENT] Splits a raw event card result text into individual display lines, splitting on roll ranges and sentences. */
